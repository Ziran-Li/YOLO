...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.loss_type == "iou":
            ...
            cw = target[:, 0] - pred[:, 0]  #中心点横坐标差
            ch = target[:, 1] - pred[:, 1]  #中心点纵坐标差
            ...
            angle_cost = 1 - 2 * torch.pow(torch.sin(torch.arcsin(sin_alpha) - math.pi / 4), 2) #原作者
            # angle_cost = torch.cos(torch.arcsin(sin_alpha) * 2 - math.pi / 2) #美团
